...
Scope is useful to use temporary variables inside the function without polluting the global scope.

Lookup rules: **LEGB** (Local, Enclosing, Global, Built-in). Python looks for variables in this order.
...
The keyword `global` is used to modify the global variable inside the function.
...
Rewrite the function without using the `global` keyword.
...
## `**kwargs` — unknown number of named arguments

Use `**kwargs` to pass an unknown number of named arguments.

Some users only have a name, while others have name, email, phone... We don't know the exact number of arguments in advance.
...
Accept a flexible number and types of arguments.
